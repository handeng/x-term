...
- 会话新增、复制、删除及自动持久化
- 串口热插拔扫描，支持常用及自定义波特率、5–8 数据位、1/1.5/2 停止位、奇偶校验和软/硬流控
- 意外断线自动重连
- ASCII/HEX 发送与显示、CR/LF/CRLF 行尾、本地回显，以及 UTF-8、ASCII、GB18030/GBK、ISO-8859-1 字符编码
- VT100/xterm 流式终端渲染，支持 ANSI 颜色、光标控制、跨数据块 UTF-8 和控制序列
- RX/TX/SYS 分向日志、毫秒时间戳、跟随尾部、复制、手动导出和实时 CSV 保存
- 手动发送历史记录，可一键恢复内容、格式、行尾和校验设置
- TXT、CSV 与二进制文件原始字节分块发送，支持进度、取消、块大小和块间隔配置
- 自动登录步骤：等待提示符、超时、延迟、ASCII/HEX 和敏感日志遮蔽
- 多条自动发送任务：连接后启动、发送间隔、有限/无限次数
- 接收帧尾/提示符触发自动回复，支持 ASCII/HEX 匹配、行尾、校验和防抖冷却
- RS-485 常用校验：CRC-16/Modbus、CRC-16/CCITT-FALSE、LRC、SUM8、XOR8，CRC 字节序可选

功能取舍与后续阶段见 [功能规划](ROADMAP.md)。
...
- 实时日志采用独立的批量写入队列，并限制待写缓存；文件发送在串口队列异步执行，慢速设备不会阻塞主界面。
